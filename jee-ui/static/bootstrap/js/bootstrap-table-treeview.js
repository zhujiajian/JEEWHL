(function ($) {
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        var indent = !item.level ? '' : sprintf('<span style="margin-left: %spx;"></span>', item.level*15);
        ...
        this.$body.find('> tr[data-index] > td > .tree-icon').off('click').on('click', function (e) {
            ...
            //递归删除子元素
            var removeChild = function (parentId,len) {
                while (len--){
                    var item = that.data[len];
                    if (parentId && item.parentId == parentId){
                        that.data.splice(len,1);
                        removeChild(item.id,that.data.length);
                    }
                }
            }
            
            if (icon.hasClass(that.options.expandIcon)) {  //收起事件
                icon.removeClass(that.options.expandIcon).addClass(that.options.collapseIcon);
                var parentId = row[that.options.treeId];
                removeChild(parentId,that.data.length);
            } else {  //展开事件
                icon.removeClass(that.options.collapseIcon).addClass(that.options.expandIcon);
                $.when(JEE.myAjax(that.options.url,{parentId:row[that.options.treeId],pageSize:-1},null,null,false)).done(function(result){
                	if(result){
                		for(var i = result.length -1;i >=0 ;i--){
                			result[i].level = row.level+1;
	            			that.data.splice(index+1,0,result[i]);
                		}
                	}
                })
            }
